...
const STATES = {
  ...
  uitgaven:{ seed: (() => { const o=JSON.parse(seed); o.settings.showPrices=true; const d=(n)=>new Date(Date.now()-n*86400000).toISOString(); o.history=[{id:"h1",at:d(1),count:9,total:42.15,paid:44.9,list:"local",items:[]},{id:"h2",at:d(8),count:12,total:61.3,paid:null,list:"local",items:[]},{id:"h3",at:d(15),count:7,total:33.8,paid:35,list:"local",items:[]},{id:"h4",at:d(29),count:10,total:55.2,paid:null,list:"local",items:[]},{id:"h5",at:d(43),count:11,total:48,paid:52.4,list:"local",items:[]}]; return JSON.stringify(o); })(), run: async p => { await p.click(".topbar .gear"); await p.waitForTimeout(450); await p.evaluate(() => { const s=[...document.querySelectorAll("#meer-content .section")].find(x=>/uitgaven/i.test(x.textContent)); if(s) s.scrollIntoView(); }); await p.waitForTimeout(200); } },
  intro:  { seed: (() => { const o=JSON.parse(seedEmpty); o.settings.seenIntro=false; return JSON.stringify(o); })(), run: async p => { for(let i=0;i<3;i++){ await p.click("#intro-next"); await p.waitForTimeout(150); } await p.click("#intro-chips .chip:nth-child(1)"); await p.click("#intro-chips .chip:nth-child(3)"); await p.waitForTimeout(300); } },
  aandeslag:{ seed, run: async p => { await p.evaluate(() => { document.querySelectorAll("#open-list .check")[0].click(); }); await p.waitForTimeout(400); } },
  barcode:{ seed, run: async p => { await p.evaluate(() => { const scr=document.querySelector("#barcode-screen"); scr.innerHTML=""; }); await p.evaluate(() => { const btn=document.querySelector("#scan-btn"); btn.style.display=""; }); await p.evaluate(() => { window.__openScan = true; }); await p.evaluate(() => { const scr=document.querySelector("#barcode-screen"); scr.classList.add("show"); scr.innerHTML="<div class=ss-badge></div><div class=eyebrow>Scan een product</div><h1>Richt op de streepjescode</h1><div class=ss-sub>Of scan de QR-code van een huisgenoot om mee te doen.</div><div id=bc-reader></div><div class=bc-status id=bc-status></div><div class=bc-hit id=bc-hit></div>"; window.showBarcodeHit({name:"Halfvolle melk",brand:"Campina",quantity:"1 l",cat:"zuivel-eieren"}); }); await p.waitForTimeout(300); } },
  typing: { seed, run: async p => { await p.fill("#add-name", "melk"); await p.waitForTimeout(300); } },
  // Fase 4/5/3C: "wat is op", winkelmodus met OP, account-blok en -blad
  op:     { seed: (() => { const o=JSON.parse(seed); const ago=(m)=>new Date(Date.now()-m*60000).toISOString(); o.list=o.list.map(i=>Object.assign({}, i, (i.name==="eieren"?{flaggedAt:ago(120),flaggedBy:"Sanne"}:(i.name==="wc papier"?{flaggedAt:ago(5),flaggedBy:"Ik"}:{})))); return JSON.stringify(o); })(), run: async () => {} },
  winkelop:{ seed: (() => { const o=JSON.parse(seed); const ago=(m)=>new Date(Date.now()-m*60000).toISOString(); o.list=o.list.map(i=>Object.assign({}, i, (i.name==="eieren"?{flaggedAt:ago(120),flaggedBy:"Sanne"}:{}))); return JSON.stringify(o); })(), run: async p => { await p.click(".shop-enter-btn"); await p.waitForTimeout(450); } },
  account:{ seed, run: async p => { await p.click(".topbar .gear"); await p.waitForTimeout(450); await p.evaluate(() => { const s=[...document.querySelectorAll("#meer-content .section")].find(x=>/^account/i.test(x.textContent.trim())); if(s) s.scrollIntoView(); }); await p.waitForTimeout(200); } },
  accountsheet:{ seed, run: async p => { await p.evaluate(() => window.openAccountSheet("link")); await p.waitForTimeout(450); } }
};
