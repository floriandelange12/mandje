...
function renderFriendsSheet(){
  ...
  var friendsHtml=(Cloud.friends||[]).map(function(f){
    return '<div class="ls-item" data-friend="'+f.user_id+'">'+
      avatarHtml(f.name, f.color, f.emoji, 38)+
      '<div class="lsi-name" style="flex:1;min-width:0;font-size:16px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+escapeHtml(f.name)+'</div>'+
      '<button class="lsi-member-action friend-send" data-friend="'+f.user_id+'" type="button" style="color:var(--green);font-weight:700">Stuur</button>'+
    '</div>';
  }).join("");
  var html='<div class="grip"></div>'+
    '<h3>Vrienden</h3>'+
    '<div class="friend-me">'+avatarHtml(me.display_name, me.color, me.emoji, 44)+
      '<div style="flex:1;min-width:0"><div style="font-weight:700;font-size:16px">'+escapeHtml(me.display_name||"Jij")+'</div>'+
      '<div style="font-size:13px;color:var(--ink-soft)">Jouw vriendcode: <b>'+escapeHtml(code||"…")+'</b></div></div>'+
    '</div>'+
    '<button class="mbtn" id="fr-share" style="background:var(--green);color:var(--on-green);border-color:transparent">Deel je vriendcode</button>'+
...
function openShareActionSheet(){
  ...
  var html='<div class="grip"></div>'+
    ...
    '<button class="sa-row" id="sa-friends">'+
      '<span class="sa-ico"><span style="font-size:19px;line-height:1">👥</span></span>'+
    ...
    '<button class="sa-row" id="sa-add">'+
      '<span class="sa-ico"><span style="font-size:17px;font-weight:800;line-height:1">🔗</span></span>'+
...
/* Avatar-render: emoji indien gekozen, anders initialen-cirkel in de kleur.
   Eén helper voor leden, vrienden en de switcher. */
var AVATAR_EMOJI = ["😀","😎","🥳","🤓","😺","🐶","🦊","🐻","🐼","🐧","🦉","🦄","🌟","🌈","🍀","🌸","🔥","⚡","🍩","🍕","🥑","☕","🎈","🎧","⚽","🚀","👑","💎","🌊","🍋"];
function avatarHtml(name, color, emoji, size){
  size = size || 34;
  var inner, bg;
  if(emoji){ inner = '<span class="emoji" style="font-size:'+Math.round(size*0.56)+'px">'+emoji+'</span>'; bg = "color-mix(in srgb, "+color+" 18%, var(--surface))"; }
  else { inner = '<span style="color:#fff;font-weight:700;font-size:'+Math.round(size*0.4)+'px;letter-spacing:.02em">'+escapeHtml(initials(name))+'</span>'; bg = color; }
  return '<span class="avatar" style="width:'+size+'px;height:'+size+'px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;flex:0 0 auto;background:'+bg+'">'+inner+'</span>';
}

/* vraag eenmalig naam + kleur + avatar-emoji; daarna callback */
function ensureIdentity(cb){
  if(Cloud.me && Cloud.me.display_name){ cb(); return; }
  var color=Cloud.me ? Cloud.me.color : pickColor();
  var chosen=color, chosenEmoji="";
  ...
  var emojis='<button class="ep-cell on" data-em="" type="button" aria-label="Initialen">Aa</button>'+
    AVATAR_EMOJI.map(function(em){ return '<button class="ep-cell" data-em="'+em+'" type="button">'+em+'</button>'; }).join("");
  var html='<h3>Hoe heet je?</h3>'+
    '<div class="frow"><input class="txt" id="id-name" placeholder="Je naam" autocapitalize="words"></div>'+
    '<div class="sheet-label"><span class="lbl-cap">Kleur</span></div>'+
    '<div class="cadrow" id="id-colors" style="gap:10px">'+dots+'</div>'+
    '<div class="sheet-label"><span class="lbl-cap">Avatar</span></div>'+
    '<div class="emoji-picker" id="id-emojis">'+emojis+'</div>'+
  ...
  s.querySelectorAll("#id-emojis .ep-cell").forEach(function(b){
    b.addEventListener("click",function(){ chosenEmoji=b.dataset.em; s.querySelectorAll("#id-emojis .ep-cell").forEach(function(x){x.classList.remove("on");}); b.classList.add("on"); });
  });
  s.querySelector("#id-go").addEventListener("click",function(){
    ...
    Cloud.me={display_name:nm, color:chosen, emoji:chosenEmoji}; Cloud.saveMe(); closeSheet2();
    // Profiel server-side aanmaken/bijwerken (inbox-lijst + friend_code), dan door
